...
export default function StripeConnectButton({ onConnect }) {
  ...
  const checkAccountStatus = async () => {
    try {
      setLoading(true);
      const { data } = await axios.get('https://agri-backend-rust.vercel.app/api/stripe/account/status', {
  ...
  const handleConnect = async () => {
    try {
      ...
      const endpoint = 'https://agri-backend-rust.vercel.app/api/stripe/connect';
